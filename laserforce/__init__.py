...
def get_achievements(id: str, amount: int):
    """
...
def get_missions(id: str, amount: int):
    """
...
def get_leaderboard(amount, type: str ="games"):
...
def get_leaderboard_from_id(id: str, type: str ="games"):
    """Grabs summary from iplaylaserforce.com (type can be games or score)"""
    id = str(id).replace("-", " ", 2)
    id = id.split()
    if type == "games":
